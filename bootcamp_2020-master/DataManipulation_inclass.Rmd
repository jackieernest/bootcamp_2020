...
```{r}
setwd("~/Dropbox/PSPG_paperwork/Bootcamp_2020/bootcamp_2020/bootcamp_2020-master/")
...
deidentified <- select(PKdata, -PT_BIRTHDATE, -PT_USUBJID, -PT_ADDRESS)
head(deidentified)
...
Summary_stats <- PKdata %>%
  summarise(c_mean = mean(DV),
            ...
            BW_lower = quantile(BW, probs = 0.025))
...
Med <- PKdata %>%
  ...
  summarise(median = median(DV),
            upper = quantile(DV, probs=0.95), #Upper 95th percentile
            lower = quantile(DV, probs=0.05)) #Lower 95th percentile
...
#Let's plot it
ggplot(DOSE_Med, aes(x=TIME, y=median, color = DOSE, fill=DOSE)) +
  geom_line() +
  geom_ribbon(aes(x=TIME, ymin=lower, ymax=upper, group=DOSE),alpha=0.3)
```
...
ggplot(HIV_Med) +
  ...
  geom_ribbon(aes(x=TIME, ymin=lower, ymax=upper, fill=factor(HIV)), alpha=0.3) +
  facet_wrap(.~DOSE)
```


## For next session, we will need to install PKPDsim
```{r}
