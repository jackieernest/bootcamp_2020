...
ggplot(DATA_GOES_HERE) + 
 geom_line(aes(x=____,y=_____,group=factor(ID)),alpha = 0.5, linetype=2,size=0.1)
...
BW_by_SEX <- PKdata %>%
  group_by(SEX) %>%
  summarise(BW_mean = _______, #Copy and paste from above
            BW_upper = ________,
            BW_lower = _________)
...
ggplot(BW_by_SEX) +
  ...
  geom_errorbar((aes(x=SEX, ymin=BW_lower, ymax=_____)))
...
Med <- PKdata %>%
  ...
  summarise(median = median(DV),
            upper = quantile(_________), #Upper 95th percentile
            lower = quantile(_________)) #Lower 95th percentile
...
DOSE_Med <- PKdata %>%
  group_by(____,TIME) %>%
  summarise(median = median(DV),
            upper = _________, #Upper 95th percentile
            lower = _________) #Lower 95th percentile

#Let's plot it
ggplot(DOSE_Med) +
  geom_line(aes(x=TIME, y=median, color = _______)) +
  geom_ribbon(aes(x=TIME, ymin=lower, ymax=upper, fill = _____), alpha=0.3)
```
